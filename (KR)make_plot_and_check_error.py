# domain_count_and_persona_error_check.py
        ...
        for scen_dict in _walk_personas(domain_block):
            for scenario, det in scen_dict.items():
                ...
                rows.append({
                    ...
                    "difficulty": det.get("difficulty", "unknown"),  # "하", "중", "상" 그대로
                    "scenario": scenario,
                    "answer": ans,  # "left", "right" 그대로
                })
...
def make_domain_plot(counts_df: pd.DataFrame, domain: str, outdir: Path) -> None:
    ...
    ax.bar([i - width/2 for i in x], counts_df.get("left", pd.Series(0, index=x)), width, label="left", color="#4C72B0")
    ax.bar([i + width/2 for i in x], counts_df.get("right", pd.Series(0, index=x)), width, label="right", color="#DD8452")
...
def check_missing(counts_df: pd.DataFrame, persona_cnt: int) -> pd.DataFrame:
    ...
    scen_tot["Total"] = scen_tot.get("left", 0) + scen_tot.get("right", 0)
...
def check_persona(pfile: Path) -> List[Dict[str, str]]:
    ...
    for sc, det in merged.items():
        ...
        ans_norm = str(ans_raw).strip().lower()
        if ans_norm == "":
...
def main() -> None:
    ...
    if args.check_error:
        ...
        print(f"문제 발견 persona 수: {df['persona_id'].nunique()}개")

...
# ALL PERSONAS (노페르소나) 데이터 (Left % / Right %)
nopersona_left = {
    1: [6.7] * 10,
    2: [83.3] * 10,
    3: [100.0] * 10,
...
def make_plot(scenarios, filename):
    ...
    for i, scenario in enumerate(scenarios):
        ...
        # LEFT 비교
        ax_left = axes[i, 0]
        bars_np = ax_left.bar(x - width/2, nopersona_vals_left, width, label='노페르소나')
        ...
        # RIGHT 비교
        ax_right = axes[i, 1]
        bars_np_r = ax_right.bar(x - width/2, nopersona_vals_right, width, label='노페르소나')
    ...
    # 전체 제목 (논문 스타일)
    fig.suptitle('도메인별 페르소나 vs. 노페르소나 CR2002 실험 결과 비교', fontsize=18)

    # Figure 전체의 legend 추가 (오른쪽 상단, 전체 바깥)
    fig.legend(['노페르소나', '페르소나'], loc='upper right', bbox_to_anchor=(1.15, 1), fontsize=12)
